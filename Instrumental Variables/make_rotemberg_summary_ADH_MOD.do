...
*** Note: Modifications to the do-file (besides the path changes) will be tagged for better understanding with either 
// Reference Note
...
gen agg_g = G2 / alpha1
rename ind sic
merge 1:1 sic using "C:\Users\Usuario\Documents\Bocconi MSc Economics and Social Sciences\Second Semester\Microeconometrics\PS2\stata_files\exercise_2\sic_code_desc"
rename sic ind
keep if _merge == 3
...
file open fh  using "rotemberg_summary_adh.tex", write replace
...
quietly {
** Default Graph ***
kdensity alpha1, normal addplot(histogram alpha1, width(0.001) color(green))
graph save pset_2_exercise_2_question_2_a.jpg, replace

** Second Iteration, removing extreme values for comprehension
kdensity alpha1 if alpha1<0.05, normal addplot(histogram alpha1 if alpha1<0.05, width(0.001) color(green)) 
graph save pset_2_exercise_1_question_2_a_alternative.jpg, replace
}
...
foreach ind in 3571 3944 3651 3661 3577 {
   ...
   local alpha_`ind' = string(r(mean), "%9.3f")
	...
	drop `temp'
	local share_beta_`ind' = string(`alpha_`ind''*`beta_`ind''/b[1,1], "%9.3f") // Note: this line was added for part 2.b
	}
...
graph export "overid_ADH.pdf", replace
...
graph export "F_vs_rotemberg_weight_ADH.pdf", replace
...
file write fh  " & $\hat{\alpha}_{k}$ & \$g_{k}$ & $\hat{\beta}_{k}$ & 95 \% CI & Ind Share & hat{\beta} share \\ \cmidrule(lr){2-6}" _n
foreach ind in 3571 3944 3651 3661 3577 {
	if `ci_min_`ind'' != -10 & `ci_max_`ind'' != 10 {
		file write fh  "`ind_name_`ind'' & `alpha_`ind'' & `g_`ind'' & `beta_`ind'' & (`ci_min_`ind'',`ci_max_`ind'')  & `share_`ind'' & `share_beta_`ind'' \\  " _n
		}
	else  {
		file write fh  "`ind_name_`ind'' & `alpha_`ind'' & `g_`ind'' & `beta_`ind'' & \multicolumn{1}{c}{N/A}  & `share_`ind'' & `share_beta_`ind'' \\ " _n
		}
	}
...
-----------------------*/
quietly {
file open q2 using "pset_2_exercise_2_question_2_b.tex", write replace



file write q2 "\textbf{Panel A: Estimates of $\beta_{k}$ for positive and negative weights}\\" _n
file write q2 "\begin{tabular}{lccc}" _n
file write q2  " & $\alpha$-weighted Sum & Share of overall $\beta$ & Mean  \\ \cmidrule(lr){2-3}" _n
file write q2  " Negative & `agg_beta_neg' & `agg_beta_neg_share' &`agg_beta_neg2' \\" _n
file write q2  " Positive & `agg_beta_pos' & `agg_beta_pos_share' & `agg_beta_pos2' \\" _n
file write q2  "\end{tabular}" _n _n



file write q2 "\textbf{Panel B: Top 5 Rotemberg weight industries} \\" _n
file write q2 " \begin{tabular}{lcccccc}" _n
file write q2 "& $\hat{\alpha}_{k}$ & \$g_{k}$ & $\hat{\beta}_{k}$ & 95 \% CI & Ind Share & $\hat{\beta}$ share \\ \cmidrule(lr){1-7}" _n
 foreach ind in 3571 3944 3651 3661 3577 {
	if `ci_min_`ind'' != -10 & `ci_max_`ind'' != 10 {
		file write q2  "`ind_name_`ind'' & `alpha_`ind'' & `g_`ind'' & `beta_`ind'' & (`ci_min_`ind'',`ci_max_`ind'')  & `share_`ind'' & `share_beta_`ind'' \\  " _n
		}
	else  {
		file write q2  "`ind_name_`ind'' & `alpha_`ind'' & `g_`ind'' & `beta_`ind'' & \multicolumn{1}{c}{N/A}  & `share_`ind'' & `share_beta_`ind'' \\ " _n
		}
	}    
file write q2 "\end{tabular}"
file close q2 
